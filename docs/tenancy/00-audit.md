...
2. **Slug do tenant inicial (backfill dos dados reais) → `zeloo`, servido no domínio raiz (`zeloo.net`), sem subdomínio próprio.** Descartadas tanto `zeloo.zeloo.net` (URL atípica) quanto `app.zeloo.net` (reservado pelo spec como domínio central de login/seleção de tenant — §2, §14, §17 — e "app" está na lista de slugs proibidos do §11, então usá-lo pra um tenant real quebraria essa reserva). Decisão confirmada: `zeloo.net`/`www.zeloo.net` continuam servindo a operação do tenant inicial (dados reais já em produção), `app.zeloo.net` fica livre pro login central/seleção de tenant conforme o desenho original do spec. `tenants.slug = "zeloo"` é o identificador interno no banco; a resolução desse tenant específico na Fase 2 (Tenant Resolver) precisa de um caso especial pra domínio raiz → tenant fixo, em vez de extração de subdomínio (detalhe de implementação pra Fase 1/2, não decidido aqui).
