...
import { clientIdSchema } from "@/modules/clients/schemas/client.schema";
import { ClientService } from "@/modules/clients/services/client.service";

const MAX_FILE_SIZE_BYTES = 8 * 1024 * 1024;
const ALLOWED_MIME = new Set(["image/png", "image/jpeg", "image/jpg", "image/webp"]);

export async function updateClientPhotoAction(formData: FormData) {
  await requireUserId();
  await requirePermission(PERMISSIONS.clients.update);

  const { id } = clientIdSchema.parse({ id: formData.get("id") });
