...
export interface BarberBarChartProps {
  data: NamedTotal[];
}
...
function BarberTooltip({ active, payload, label }: TooltipContentProps) {
...
export function BarberBarChart({ data }: BarberBarChartProps) {
  if (data.length === 0) {
  ...
  return (
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
        ...
        />
        <Tooltip content={<BarberTooltip />} cursor={{ fill: "rgba(255,255,255,0.04)" }} />
