...
export async function updateOgImageAction(formData: FormData) {
  await requireUserId();
  ...
  try {
    ...
    const settings = await service.updateOgImage({ buffer, originalName: file.name, mimeType: file.type });
